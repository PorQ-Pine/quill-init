...
pub fn start_usbnet() -> Result<()> {
    ...
    // USB networking
    run_command("ifconfig", &[&iface_name, "up"]).with_context(|| "Failed to activate {iface_name}")?;
    run_command("ifconfig", &[&iface_name, &IP]).with_context(|| "Failed to set IP for {iface_name}")?;
...
pub fn prepare_script_login(pubkey: &PKey<Public>) -> Result<()> {
    ...
    let script_path = format!("{}{}{}", &libqinit::DATA_PART_MOUNTPOINT, &libqinit::BOOT_DIR, &DEBUG_SETUP_SCRIPT);
    let script_signature_path = format!("{}.dgst", &script_path);
    if fs::exists(&script_path)? && check_signature(&pubkey, &script_path, &script_signature_path)? {
